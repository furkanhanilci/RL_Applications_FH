...
if mode == 0:  # Training mode
    ...
    for episode in range(num_episodes):
        ...
        print(
            f"Episode {episode}, Reward: {episode_reward}, Success Rate: {success_rate}, Average Reward: {average_reward}")
    ...
    # Plotting the graphs with reduced sample size
    sample_interval = 10  # Adjust this value to reduce the number of samples plotted
    sampled_episodes = range(0, num_episodes, sample_interval)

    fig, axs = plt.subplots(4, 1, figsize=(12, 20))

    axs[0].plot(sampled_episodes, [rewards[i] for i in sampled_episodes], label='Reward')
    axs[0].set_title('Reward per Episode')
    axs[0].set_xlabel('Episode')
    axs[0].set_ylabel('Reward')
    axs[0].legend()

    axs[1].plot(sampled_episodes, [success_rates[i] for i in sampled_episodes], label='Success Rate')
    axs[1].set_title('Success Rate per Episode')
    axs[1].set_xlabel('Episode')
    axs[1].set_ylabel('Success Rate')
    axs[1].legend()

    axs[2].plot(sampled_episodes, [average_rewards[i] for i in sampled_episodes], label='Average Reward')
    axs[2].set_title('Average Reward per Episode')
    axs[2].set_xlabel('Episode')
    axs[2].set_ylabel('Average Reward')
    axs[2].legend()

    axs[3].plot(sampled_episodes, [running_times[i] for i in sampled_episodes], label='Running Time')
    axs[3].plot(sampled_episodes, [convergence_times[i] for i in sampled_episodes], label='Convergence Time')
